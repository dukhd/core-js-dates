...
function getCountDaysInMonth(month, year) {
  if (month === 2) {
    ...
    const isLeapYearCheck = date.getDate() === 29;
    if (isLeapYearCheck) return 29;
    return 28;
  }
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  ...
  return Math.round(diffInMs / oneDayMs) + 1;
}
...
function getWeekNumberByDate(date) {
  ...
  day.setDate(day.getDate() + 4 - currentDay);
  const yearStart = new Date(day.getFullYear(), 0, 1);
  const diffInDays = (day - yearStart) / (1000 * 3600 * 24);
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const current = new Date(startParts[2], startParts[1] - 1, startParts[0]);
